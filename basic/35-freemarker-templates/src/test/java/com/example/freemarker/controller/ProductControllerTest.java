 ...
 * <ul>
 * <li>Starts only the <em>web layer</em> (Spring MVC, FreeMarker view
 ...
 *
 * <p>
 * FreeMarker note: {@code @WebMvcTest} also auto-configures the FreeMarker
 * {@code ViewResolver}, so the actual {@code .ftlh} templates are rendered
...
class ProductControllerTest {
    ...
    void getIndex_noFilter_returns200AndIndexView() throws Exception {
        ...
        mockMvc.perform(get("/"))
                ...
                // (Spring MVC resolves it to index.ftlh via the FreeMarker ViewResolver)
    ...
    void getProductDetail_existingId_returnsDetailView() throws Exception {
        ...
        mockMvc.perform(get("/products/1"))
                ...
                // The rendered FreeMarker template must include the product name
                .andExpect(content().string(containsString("Wireless Headphones")));
    }
    ...
    void getInStock_templateRendersCorrectCount() throws Exception {
        ...
        mockMvc.perform(get("/products/in-stock"))
                .andExpect(status().isOk())
                // FreeMarker renders ${products?size} → "3" inside the count badge
