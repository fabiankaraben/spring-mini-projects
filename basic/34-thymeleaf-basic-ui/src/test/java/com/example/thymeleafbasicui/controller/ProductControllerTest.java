 ...
 * <ul>
 * <li>Starts only the <em>web layer</em> (Spring MVC, Thymeleaf, filters,
 * etc.).</li>
 * <li>Does NOT load the full application context — no data-source, no cache,
 * etc.</li>
 * <li>Automatically configures a {@link MockMvc} instance for making fake HTTP
 * requests.</li>
 * </ul>
...
class ProductControllerTest {
    ...
    void getIndex_noFilter_returns200AndIndexView() throws Exception {
        ...
        mockMvc.perform(get("/"))
                ...
                // The logical view name resolved by the controller must be "index"
    ...
    void getProductDetail_unknownId_returnsNotFoundView() throws Exception {
        ...
        mockMvc.perform(get("/products/999"))
                .andExpect(status().isOk()) // HTTP 200 — the controller renders a page, not a 404
    ...
    @Test
    @DisplayName("GET /products/in-stock shows all in-stock items count in the page")
    void getInStock_templateRendersCorrectCount() throws Exception {
        ...
        mockMvc.perform(get("/products/in-stock"))
                .andExpect(status().isOk())
                // Thymeleaf renders ${#lists.size(products)} → "3" inside the count badge
