package com.example.activemqjms.domain;
...
/**
 * Domain model representing an order message that travels through the JMS broker.
 *
 * <p>This class is the payload serialised to JSON and published to the ActiveMQ queue.
 * Downstream consumers ({@link com.example.activemqjms.service.MessageConsumerService})
 * receive and deserialise this object.
 *
 * <h2>Design notes</h2>
 * <ul>
 *   <li><strong>messageId</strong>: a UUID auto-generated at construction time that
 *       uniquely identifies this message. Consumers can use it for idempotency checks
 *       (i.e. ignore duplicate deliveries caused by re-delivery after a consumer crash).</li>
 *   <li><strong>createdAt</strong>: an ISO-8601 timestamp captured at construction time.
 *       Useful for auditing, dead-letter analysis, and SLA tracking.</li>
 *   <li>Implements {@link Serializable} as a safeguard; in practice the message is
 *       serialised to JSON via Jackson, not Java object serialisation.</li>
 * </ul>
 */
public class OrderMessage implements Serializable {

    /**
     * Unique identifier for this message, auto-generated as a random UUID string.
     ...
     *
     * <p>Jackson uses reflection to instantiate an empty object and then populates
     * fields via setter methods. Without this constructor Jackson cannot deserialise
     * JSON back into an {@code OrderMessage}.
     */
    public OrderMessage() {
    }

    /**
     * Full constructor used by the service layer to create a new outgoing message.
     *
